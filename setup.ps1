...
function Test-InternetConnection {
    try {
        Test-Connection -ComputerName www.vg.no -Count 1 -ErrorAction Stop | Out-Null
        return $true
    }
    catch {
...
function Install-NerdFonts {
    ...
    try {
        ...
        Write-Error "Failed to download or install ${FontDisplayName} font."
    }
}
...
if (!(Test-Path -Path $PROFILE -PathType Leaf)) {
    try {
        ...
        Invoke-RestMethod https://github.com/espenakk/pwsh/raw/main/Microsoft.PowerShell_profile.ps1 -OutFile $PROFILE
